...
(function(page){
	...
	page.init = function() {
		page.bindEvent();
		showScreenApps(0,application,userId);
		setMarginLeft(0);
		setScreenSetting();
	}
	...
	function saveDragPosition() {
		...
		$.ajax({
			...
			url: interface.saveDragPosition,
			success: function() {
				alert("保存成功！");
			},
			error: function() {
	...
	page.bindEvent = function() {
		...
		//删除按钮点击事件
		$appList.on('click', '.delete-btn',function(){
			var appPages = $('.app-page');
			var appList = $(appPages[nowScreen]).find('.app-list');
			if(appList.find('.app-demo').length <= 1) {
				alert("该屏幕菜单数不能小于1个，不能再删除！");
				return;
			}
			var parent=$(this).parent();
			$.ajax({
	             type: "GET",
	             url: interface.delApps,
	             data: {menuid:$(this).parent().attr('data-id'), userId:userId},
	             dataType: "json",
	...
	function screenDeleteEvent() {
		...
		$screenSetting.on('click', 'button', function(){
			...
			alert('index:'+index);
			alert('screenNum:'+screenNum);
			if(confirm('确认删除该屏幕吗？')) {
				var parent = $(this).parent().parent();
				$.ajax({
		             type: "GET",
		             url: interface.delScreen,
		             data: {userId:userId,screenIndex:index,application:application},
		             dataType: "json",
		             success: function(data){
		            	 		if(1===data){
		            	 			alert("删除失败！");
	...
	function circleEvent() {
		$circlesParent.on('click', 'img', function(){
			...
			if(index > 0 && index < $('.app-guide img').length - 1) { // 因为最后一张图片不是小圆点，所以过滤掉
				...
				showScreenApps(index - 1,application,userId); //因为第一张图片不是小圆点，所以减1
	...
	function appGroupEvent() {
		$sidebarMiddle.on('click', 'li', function(){
			...
			findApplistByGroup(menuid,userId,application);
		});
	}
	...
	function dialogAppListEvent() {
		$dialogAppList.on('click', '.group-one-app', function() {
			...
			if(appList.find('.app-demo').length >= 21) {
				alert("该屏幕菜单数不能超过21，不能再添加！");
				return;
			}
			var me = $(this);
			var imgUrl = me.find('img').attr('src');
			var appTitle = me.find('.one-app-title').html();
			var appId=me.find('img').attr('id');
			var menuUrl = me.find('img').attr('data-url');
//			me.remove();
//			addAppToScreen(imgUrl, appTitle,appId,menuUrl);
			$.ajax({
	             type: "GET",
	             url: interface.insertApps,
	             data: {menuid:appId, userId:userId},
	             dataType: "json",
	             success: function(data){
	            	 		if(1===data){
	            	 			alert("添加失败！");
	...
	//添加app应用到屏幕
	function addAppToScreen(imgUrl, appTitle,appId,menuUrl){
		var appPages = $('.app-page');
		var appList = $(appPages[nowScreen]).find('.app-list');
		var html = '<div class="app-demo" data-id="'+appId+'">'
				 +		'<button class="delete-btn">移除</button>'
				 +		'<div class="app-img">'
				 +			'<img src="'+imgUrl+'" alt="'+appTitle+'"  id="'+appId+'"/>'
				 +		'</div>'
				 +		'<div class="app-title">'+appTitle+'</div>'
				 +	'</div>';
		appList.append(html);
		addHoverEvent();
		alretApp(appId,menuUrl);
		initDrag();
	...
	function alertAppList() {
		...
		getAppGroupList(application,userId);
		showAppSetting();
	}
	 ...
	 */
	function showScreenApps(index,application,userId) {
		...
		$.getJSON(
			...
			function(data) {
				...
				for(var i = 0, len = list.length; i < len; i++) {
					...
					html = '<div class="app-demo" data-id="'+menuid+'">'
						 +		'<button class="delete-btn">移除</button>'
						 +		'<div class="app-img">'
						 +			'<img src="'+list[i].imgUrl+'" alt="'+list[i].imgTitle+'"  id ="'+menuid+'"/>'
						 +		'</div>'
						 +		'<div class="app-title">'+list[i].imgTitle+'</div>'
						 +	'</div>';
					htmlArr.push(html);
					menus[menuid] = list[i].menuUrl ;
				}
				appList.html(htmlArr.join(''));
				for(var key in menus) {
					if(typeof key === "string"){
						alretApp(key,menus[key]);
					}
				}
				addHoverEvent();
				initDrag();
	 ...
	 * 打开相应Tab
	 */
	function alretApp(menuid,menuUrl){
		$('#'+menuid).unbind("dblclick");
		$('#'+menuid).bind("dblclick",function(){
		    /*var event=new zk.Event(zk.Widget.$(jq('$myDesktop')),'onTabDesk',{'menuUrl':menuUrl},{toServer:true});
			zAu.send(event);*/
			clickMenu(menuUrl);
		});	
	 ...
	 * @description 弹框中，应用设置相关函数
	 ==================================================================================*/
	//获取app分类列表
	function getAppGroupList(application,userId) {
		$.getJSON(
			interface.getAppGroupList,
			{
				application:application,
				userId:userId
			},
			function(data){
				...
				for(var i = 0, len = list.length; i < len; i++) {
					html = '<li id="'+list[i].menuId+'"><img src="'+list[i].iconUrl+'"/>'+list[i].groupTitle+'</li>';
					htmlArr.push(html);
				}
				htmlArr.push('</ul>');
	...
	//根据app分类Id， 查询app列表
	function findApplistByGroup(menuid,userId,application) {
		$.getJSON(
			interface.getBoxGroupList,
			{
				menuid:menuid,
				userId:userId,
				application:application
			...
			function(data) {
				...
				var list = data.img;
				for(var i = 0, len = list.length; i < len; i++) {
					html = '<div class="group-one-app" data-id="'+list[i].menuId+'"  >'
						 +		'<div class="one-app-img">'
						 +				'<img src="'+list[i].imgUrl+'" alt="'+list[i].imgTitle+'" id="'+list[i].menuId+'" data-url="'+list[i].menuUrl+'"/>'
						 +			'</div>'
						 +			'<div class="one-app-title">'+list[i].imgTitle+'</div>'
						 +		'</div>';
					htmlArr.push(html);
	...
	function addScreen() {
		...
		//在桌面上面添加新的屏幕
		$('#app-pages').append('<div class="app-page"><div class="app-list"></div></div>');
